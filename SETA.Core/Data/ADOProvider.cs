...
using SETA.Core.Helper.Attributes;
...
namespace SETA.Core.Data
{
    ...
    public class ADOProvider : IDisposable
    {
        ...
        public IEnumerable<T> Procedure<T>(string procedure, object oParams = null, bool useCache = true)
        {
            ...
            {
                newOParams = AutoMapping.MapParameters(oParams);
            }
            using (MiniProfiler.Current.Step(procedure))
            {
                using (Connection)
                {
                    ...
                    var key = procedure + CacheHelper.GenerateSuffixKeyByParams(newOParams);
                    ...
                    if (useCache && CacheHelper.UseCache)
                    {
                        ...
                        var obj = newOParams.GetType();
                        if (obj.FullName == "Dapper.DynamicParameters")
                        {
                            listParams = newOParams as DynamicParameters;
                            if (listParams != null)
                            {
                    ...
                    {
                        items = Connection.Query<T>(procedure, newOParams, commandType: CommandType.StoredProcedure, commandTimeout: Connection.ConnectionTimeout);
        ...
        public SqlMapper.GridReader ProcedureQueryMulti(string procedure, object oParams = null, bool useCache = true)
        {
            ...
            using (MiniProfiler.Current.Step(procedure))
            {
                ...
                using (Connection)
                {
                    ...
                    var items = Connection.QueryMultiple(procedure, oParams, commandType: CommandType.StoredProcedure, commandTimeout: Connection.ConnectionTimeout);
                    //Cache.Set(key, items);
                    return items;
                }
            }
        ...
        public bool ProcedureExecute(string procedure, object oParams = null)
        {
            try
            {
                using (MiniProfiler.Current.Step(procedure))
                {
                    using (Connection)
                    {
                        Connection.Execute(procedure, oParams, commandType: CommandType.StoredProcedure, commandTimeout: Connection.ConnectionTimeout);
                    }
                }
                return true;
            }
        ...
        public void InsertDataUsingSqlBulkCopy<T>(IEnumerable<T> data, string schema = "")
        {
            ...
            {
                Logging.PutError("Exception: " + ex.Message, ex);
            }
        }
